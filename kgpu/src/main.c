...
ssize_t kgpu_read(struct file * filp, char __user * buf, size_t c,
                  loff_t * fpos)
{
  ssize_t ret = 0;
  struct list_head * r;
...
ssize_t kgpu_write (struct file * filp, const char __user * buf, size_t count,
                    loff_t * fpos)
{
  ...
  else
  {
    ...
    memcpy(&kuresp, buf, realcount);/*copy_from_user*/
...
static int kgpu_vm_fault (struct vm_area_struct * vma, struct vm_fault * vmf)
{
  /*static struct page *p = NULL;
    if (!p) {
      ...
      {
        kgpu_log(KGPU_LOG_ERROR, "first page fault, kgpu mmap no page ");
      }
    }*/
...
static void set_vm (struct vm_area_struct * vma)
{
  ...
  kgpudev.vm.bitmap = kmalloc(BITS_TO_LONGS(kgpudev.vm.npages) * sizeof(long),
                              GFP_KERNEL);
...
static int kgpu_mmap (struct file * filp, struct vm_area_struct * vma)
{
  ...
  {
    kgpu_log(KGPU_LOG_ALERT, "mmap size incorrect from 0x$lX to 0x%lX with "
...
static int kgpu_init(void)
{
  ...
  kgpu_request_cache = kmem_cache_create ("kgpu_request_cache",
                                sizeof(struct kgpu_request), 0,
  ...
  kgpu_request_item_cache = kmem_cache_create("kgpu_request_item_cache",
                                        sizeof(struct _kgpu_request_item), 0,
                                        SLAB_HWCACHE_ALIGN,
                                        kgpu_request_item_constructor);
  ...
  kgpu_sync_call_data_cache = kmem_cache_create("kgpu_sync_call_data_cache",
                                      sizeof(struct _kgpu_sync_call_data), 0,
                                      SLAB_HWCACHE_ALIGN,
                                      kgpu_sync_call_data_constructor);
